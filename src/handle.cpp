...
namespace odbcxx {
	...
	handle const& handle::null = handle();
	...
	SQLRETURN handle::set_attribute(SQLINTEGER attribute,
			SQLPOINTER value_ptr,
			SQLINTEGER buf_len) const
	{ return check_error((*m_setter)(m_handle, attribute, value_ptr, buf_len)); }

	handle handle::alloc(handle::handle_type type) const {
	...
	SQLRETURN handle::check_error(SQLRETURN retcode) const {
